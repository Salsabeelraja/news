...
class NewsAdapter : RecyclerView.Adapter<NewsAdapter.ArticleViewHolder>() {
    ...
    private val differCallback = object : DiffUtil.ItemCallback<Article>() {
        override fun areItemsTheSame(oldItem: Article, newItem: Article): Boolean {
            return oldItem.url == newItem.url //instead of the id cause it's the room generated and the articles in itself doesn't have id , url unique like the id
        }
        ...
        holder.itemView.apply {
            ...
            setOnClickListener {
                onItemClickListener?.let {
                    //it refers to the onItemClickListener lambda function
                    it(article) // here we call lambda function with current article
                }
            }
        }
    }
